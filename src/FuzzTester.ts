...
class FuzzTester {
    ...
    async runTests() {
        ...
        console.log(YELLOW + "[TEST] Split (1 -> 20)" + RESET);
        const tSplit = this.randToken();
        const sSplit = this.buildScenario({
            inputs: [{ amount: 1000n, tokenId: tSplit }],
            outputs: Array(20).fill({ amount: 50n, tokenId: tSplit })
        });
        // Split allows outputs 1..19 to be unverified by the Input Kernel (1-to-1 limitation).
        // This is a known structural weakness of the current kernel.
        // If it passes, it means outputs 1..19 are valid *structures* but unconstrained by lineage.
        console.log(`Result: ${await this.runVM(sSplit.tx, sSplit.sourceTxs) ? GREEN + "PASS (Dangerous)" : RED + "FAIL"}` + RESET);

        console.log(YELLOW + "[TEST] Merge (20 -> 1)" + RESET);
        const tMerge = this.randToken();
        const sMerge = this.buildScenario({
            inputs: Array(20).fill({ amount: 50n, tokenId: tMerge }),
            outputs: [{ amount: 1000n, tokenId: tMerge }]
        });
        // Merge should FAIL because Output0 cannot satisfy 20 different inputs simultaneously.
        console.log(`Result: ${await this.runVM(sMerge.tx, sMerge.sourceTxs) ? GREEN + "PASS" : RED + "FAIL (Expected)"}` + RESET);

        console.log(YELLOW + "[TEST] Mixed Assets (Token A + Token B)" + RESET);
        const tA = this.randToken();
        const tB = this.randToken();
        const sMixed = this.buildScenario({
            inputs: [{ amount: 100n, tokenId: tA }, { amount: 50n, tokenId: tB }],
            outputs: [{ amount: 100n, tokenId: tA }, { amount: 50n, tokenId: tB }]
        });
        // Mixed should FAIL if Lineage Check forces Output0.TokenID == Input.TokenID for ALL inputs.
        // Input A checks Output 0 (Token A). OK.
        // Input B checks Output 0 (Token A). FAIL (TokenID mismatch).
        console.log(`Result: ${await this.runVM(sMixed.tx, sMixed.sourceTxs) ? GREEN + "PASS" : RED + "FAIL (Expected, 1-to-1 Requirement)"}` + RESET);

        console.log(YELLOW + "[TEST] Swap Attack (A -> B)" + RESET);
        const tSwapIn = this.randToken();
        const tSwapOut = this.randToken();
        const sSwap = this.buildScenario({
            inputs: [{ amount: 100n, tokenId: tSwapIn }],
            outputs: [{ amount: 100n, tokenId: tSwapOut }]
        });
        console.log(`Result: ${!await this.runVM(sSwap.tx, sSwap.sourceTxs) ? GREEN + "PASS (Attack Rejected)" : RED + "FAIL (Attack Accepted)"}` + RESET);
    }
}
